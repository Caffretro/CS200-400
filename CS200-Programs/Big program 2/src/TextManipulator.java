...
public class TextManipulator {
    ...
    public static String matchCase(String template, String original) {
        ...
        if (template.length() >= original.length()) {
            for (int i = 0; i < original.length(); ++i) {
                if (Character.isUpperCase(template.charAt(i)) == true) {
                    String partWord = original.substring(i, i + 1).toUpperCase();
                    cased = cased + partWord;
                } else {
                    String partWord = original.substring(i, i + 1).toLowerCase();
                    cased = cased + partWord;
                }
            }//match case for a string that is shorter than the template
            return cased;
            ...
            }//match case for a string that is longer than the template for the first part
            String endfulWord = original.substring(template.length()).toLowerCase();
            //turn the rest part into lower case
            cased = cased + endfulWord;//connect them
            return cased;
        }
    }
    ...
    public static String translate(String word, ArrayList<String[]> wordList) {
        ...
        for (int i = 0; i < wordList.size(); ++i) {
            contrast = word;
            if (contrast.equalsIgnoreCase(wordList.get(i)[0]) || contrast.equals("a")) {
                translated = wordList.get(i)[1];
                translated = matchCase(word, translated);
                matched = true;
                break;
            }
        }//go over the dict file and match the translated String's case
        
        if (matched == false) {
            for (int i = 0; i < word.length(); i++) {
                noMatch = noMatch + Config.LINE_CHAR;
            }
            translated = noMatch;
        }//if no translation
    ...
    public static String pigLatin(String word) {
        ...
        if (word.charAt(0) == 'a' || word.charAt(0) == 'e' || word.charAt(0) == 'i' || 
            ...
            word.charAt(0) == 'O' || word.charAt(0) == 'U' || word.charAt(0) == 'Y') {
            pigword = word + "way";
        }//if the beginning letter is vowel
        else if (Character.isLetter(word.charAt(0)) != true && word.charAt(0) != '\'') {
            pigword = word;
        }//special case of '
        else {
            while (indexOfApo < word.length()) {
                if (!(word.charAt(indexOfApo) == 'a' || word.charAt(indexOfApo) == 'e' || 
                    word.charAt(indexOfApo) == 'i' || word.charAt(indexOfApo) == 'o' || 
                    word.charAt(indexOfApo) == 'u' || word.charAt(indexOfApo) == 'y' ||
                    word.charAt(indexOfApo) == 'A' || word.charAt(indexOfApo) == 'E' || 
                    word.charAt(indexOfApo) == 'I' || word.charAt(indexOfApo) == 'O' || 
                    word.charAt(indexOfApo) == 'U' || word.charAt(indexOfApo) == 'Y')) {
                    indexOfApo++;
                }
                    ...
                    break;
                }//count how many non-vowel letters are there
                
            }
            pigword = word.substring(indexOfApo, word.length()) + 
                      word.substring(0, indexOfApo) + "ay";//put them in the back and add "ay"
        }
        pigword = matchCase(word, pigword);//then match case for the string
        return pigword;
    }
    ...
    public static String reverse(String word) {
        ...
        reversed = matchCase(word, reversed);
        return reversed;//just reverse the word
    }
    ...
    public static ArrayList<String> reverse(ArrayList<String> arrL) {
        ...
        for (int i = 0; i < arrL.size(); i++) {
            reversed.add(arrL.get(arrL.size() - 1 - i));
        }
        //add the content in the ArrayList<String> to a new one with opposite sequence
        return reversed;
    }
    ...
    public static char promptMenu(Scanner sc, String[] files, boolean[] modFlags, boolean modeBoth,
        ...
        if (showMenu == true) {
            ...
            System.out.println();
            /*if (modFlags[Config.MOD_TRANS] == false && modFlags[Config.MOD_PIG] == false &&
                modFlags[Config.MOD_REV_WORD] == false && modFlags[Config.MOD_REV_LINE] == false) {
                System.out.println();
            }*/
        ...
        char userResult = userInput.charAt(0);
        //FIXME
        return userResult;//quite straightforward 
    }
    ...
    public static String updateFileName(Scanner sc, String curFileName) {
        ...
        updatedName.trim();//maybe contains error, eliminate spaces
        return updatedName;
        //FIXME
    }
    ...
    public static void readDictFile(String fileName,
        ...
        try {
            FileInputStream dictStream = new FileInputStream(fileName);
            Scanner inDS = new Scanner(dictStream);//open and add it to scanner
            Scanner inDS2 = null;//scan the content in every line
            String sentence = "";
            
            do {
                sentence = inDS.nextLine();
                sentence.split(" ");
                inDS2 = new Scanner(sentence);
                ...
                wordList.add(line);
                inDS2.close();
            } while(inDS.hasNextLine());
            inDS.close();
            dictStream.close();
        }
        catch(IOException e) {
            throw new IOException("\"Exception: File '\" + mainFiles[Config.FILE_DICT] + \"' not found.\"");
        }
        return;
        //FIXME
    }
    ...
    public static void readInputFile(String fileName,
        ...
        try{
            ...
            Scanner inFS = new Scanner(userStream);
            Scanner inFS2 = null;
            String sentence = "";//for second scanner to scan
            String words = "";
            String element = "";
            String elementOfPunc = "";
            int arrayPlace = 0;
            
            do {
                sentence = inFS.nextLine();
                sentence.split(" ");
                inFS2 = new Scanner(sentence);
                ...
                while (inFS2.hasNext()) {
                    ...
                    for (int i = 0; i < words.length(); i++) {
                        if (Character.isLetter(words.charAt(i))) {
                            element = element + words.charAt(i);
                        }//normal case
                        else if (words.charAt(i) == '\'') {
                            element = element + words.charAt(i);
                        }//special case for '
                        else {
                            elementOfPunc = elementOfPunc + words.charAt(i);
                        }//if there is a punctuation, there will be value inside.
                    }
                    fileByLine.get(arrayPlace).add(element);//always add the word to arraylist
                    if (elementOfPunc.length() > 0) {
                        fileByLine.get(arrayPlace).add(elementOfPunc);
                    }//special case when containing punctuations, add as a new element
                }
                arrayPlace++;//move to next line
                inFS2.close();
            } while(inFS.hasNext());
            inFS.close();
            userStream.close();
            ...
            throw new IOException("\"Exception: File '\" + mainFiles[Config.FILE_IN] + \"' not found.\"");
        }
        return;
    }
     ...
     * where fileName is the name of the file that the method attempted to open.
     *
     * @param fileName The name of the output file to parse.
    ...
    public static void saveToFile(String fileName,
        ...
        try {
            ...
            PrintWriter outFS = new PrintWriter(writerStream);
            boolean firstDoubleQuote = true;
            for (int i = 0; i < modFileByLine.size(); i++) {
                for (int j = 0; j < modFileByLine.get(i).size() - 1; j++) {
                    outFS.print(modFileByLine.get(i).get(j));
                        if (Character.isLetter(modFileByLine.get(i).get(j + 1).charAt(0))) {
                            if (firstDoubleQuote == false) {
                                //ignore the space that should not be printed after the first "
                            }
                            else {
                                outFS.print(" ");
                            }
                        }
                        else if (!Character.isLetter(modFileByLine.get(i).get(j + 1).charAt(0))) {
                            if (modFileByLine.get(i).get(j + 1).charAt(0) == '\"'){
                                if (firstDoubleQuote) {
                                    outFS.print(" ");
                                ...
                                //since " goes with pairs
                            }
                        }
                    }
                outFS.print(modFileByLine.get(i).get(modFileByLine.get(i).size() - 1));
                outFS.println();
                }
            ...
            throw new IOException("\"Exception: File '\" + mainFiles[Config.FILE_OUT] + \"' not found.\"");
        }
        return;
    }
     ...
     *
     * If modeBoth is false, then the contents of modFileByLine is output line by line. Eanew line
     * character.
     *
     * For the non-interleaved mode, consider the following example:
     * modFileByLine contains 2 ArrayList of Strings: ch word is
     * output followed by a space except for the last word. Each line is terminated with a 
    ...
    public static void display(ArrayList<ArrayList<String> > fileByLine,
        ...
        if (modeBoth == false) {
            for (int i = 0; i < modFileByLine.size(); i++) {
                ...
                }//concat a space between elements
                System.out.println();//next line
            }
        }
        else {
            if (modFileByLine.size() != fileByLine.size()) {
                throw new Exception("Number of lines in modified version differs from original.");
            }
            for (int i = 0; i < modFileByLine.size(); i++) {
                for (int j = 0; j < modFileByLine.get(i).size() - 1; j++) {
                    if (modFileByLine.get(i).size() != fileByLine.get(i).size()) {
                        throw new Exception("Number of words on line " + i + 
                            " in modified version differs from original.");
                    }
                    if (modFileByLine.get(i).get(j).length() < fileByLine.get(i).get(j).length()) {
                        for (int l = 0; l < fileByLine.get(i).get(j).length() - 
                            modFileByLine.get(i).get(j).length(); l++) {
                            spaces = spaces + " ";
                        }
                    }
                    System.out.print(modFileByLine.get(i).get(j) + " " + spaces);
                    spaces = "";
                }
                System.out.print(modFileByLine.get(i).get(modFileByLine.get(i).size() - 1));
                System.out.println();
                for (int j = 0; j < fileByLine.get(i).size() - 1; j++) {
                    if (fileByLine.get(i).get(j).length() < modFileByLine.get(i).get(j).length()) {
                        for (int l = 0; l < modFileByLine.get(i).get(j).length() - 
                            fileByLine.get(i).get(j).length(); l++) {
                            spaces = spaces + " ";
                        }
                    }
                    System.out.print(fileByLine.get(i).get(j) + " " + spaces);
                    spaces = "";
                }
                System.out.print(fileByLine.get(i).get(fileByLine.get(i).size() - 1));
        ...
        //for milestone1, regardless of modeBoth
        //FIXME
    }
    ...
    public static ArrayList<ArrayList<String> > manipulate(ArrayList<ArrayList<String> > fileByLine,
        ...
        for (int i = 0; i < fileByLine.size(); i++) {
            ...
            for (int j = 0; j < fileByLine.get(i).size(); j++) {
                newList.get(i).add(fileByLine.get(i).get(j));
            }
        }
        ...
        if (modFlags[Config.MOD_TRANS] == true) {
            for (int i = 0; i < newList.size(); i++) {
                for (int j = 0; j < newList.get(i).size(); j++) {
                    newList.get(i).set(j, translate(newList.get(i).get(j), dict));
                }
            }
        }
        if (modFlags[Config.MOD_PIG] == true) {
            for (int i = 0; i < newList.size(); i++) {
                for (int j = 0; j < newList.get(i).size(); j++) {
                    newList.get(i).set(j, pigLatin(newList.get(i).get(j)));
                }
            }
        }
        if (modFlags[Config.MOD_REV_WORD] == true) {
            for (int i = 0; i < newList.size(); i++) {
                for (int j = 0; j < newList.get(i).size(); j++) {
                    newList.get(i).set(j, reverse(newList.get(i).get(j)));
                }
            }
        }
        if (modFlags[Config.MOD_REV_LINE] == true) {
            for (int i = 0; i < newList.size(); i++) {
                newList.set(i, reverse(newList.get(i)));
            }
        }
        return newList;
        //FIXME
    ...
    public static void main(String[] args) {
        ...
        boolean[] mainModFlags = new boolean[Config.NUM_MODS];
        boolean mainModeBoth = false;
        boolean mainShowMenu = true;
        String[] mainFiles = new String[Config.NUM_FILES];
        String mainCurrFileName = null;
        String mainCurrDict = null;
        ...
        char action = ' ';
        do {
            action = promptMenu(scnr, mainFiles, mainModFlags, mainModeBoth, mainShowMenu);
            //call promptMenu, returns the instruction
            try {
                    //do the action that user enters
                    if (action != 'q' && action != 'Q') {
                        switch(action) {
                            case '1':
                                ...
                                System.out.println();
                                display(fileByLine, modFileByLine, mainModeBoth);
                                for (int i = 0; i < 80; i++) {
                                    System.out.print(Config.LINE_CHAR);
                                }
                                System.out.println();
                                break;
                            case '2':
                                modFileByLine = manipulate(fileByLine, dictList, mainModFlags);
                                saveToFile(mainFiles[Config.FILE_OUT], modFileByLine);
                                break;
                            case 'i':
                            case 'I':
                                mainFiles[Config.FILE_IN] = updateFileName(scnr, mainCurrFileName);
                                if (mainFiles[Config.FILE_IN] == "") {
                                    mainFiles[Config.FILE_IN] = null;
                                    break;
                                }
                                if (mainFiles[Config.FILE_IN] != mainCurrFileName) {
                                    readInputFile(mainFiles[Config.FILE_IN], fileByLine);
                                    readInputFile(mainFiles[Config.FILE_IN], modFileByLine);
                                }
                                break;
                            case 'h':
                            case 'H':
                                if (mainShowMenu == true) {
                                    mainShowMenu = false;
                                }
                                else {
                                    mainShowMenu = true;
                                }//switch whether show menu;
                                break;
                            case 't':
                            case 'T':
                                if (mainModFlags[Config.MOD_TRANS] == true) {
                                    mainModFlags[Config.MOD_TRANS] = false;
                                }
                                else {
                                    mainModFlags[Config.MOD_TRANS] = true;
                                    modFileByLine = manipulate(fileByLine, dictList, mainModFlags);
                                }//switch whether apply translation
                                break;
                            case 'w':
                            case 'W':
                                if (mainModFlags[Config.MOD_REV_WORD] == true) {
                                    mainModFlags[Config.MOD_REV_WORD] = false;
                                }
                                else {
                                    mainModFlags[Config.MOD_REV_WORD] = true;
                                    modFileByLine = manipulate(fileByLine, dictList, mainModFlags);
                                }//switch whether apply word reverse
                                break;
                            case 'l':
                            case 'L':
                                if (mainModFlags[Config.MOD_REV_LINE] == true) {
                                    mainModFlags[Config.MOD_REV_LINE] = false;
                                }
                                else {
                                    mainModFlags[Config.MOD_REV_LINE] = true;
                                    modFileByLine = manipulate(fileByLine, dictList, mainModFlags);
                                }
                                break;
                            case 'd':
                            case 'D':
                                mainFiles[Config.FILE_DICT] = updateFileName(scnr, mainCurrDict);
                                if (mainFiles[Config.FILE_DICT] == "") {
                                    mainFiles[Config.FILE_DICT] = null;
                                    break;
                                }
                                if (mainFiles[Config.FILE_DICT] != mainCurrFileName) {
                                    readDictFile(mainFiles[Config.FILE_DICT], dictList);
                                }
                                break;
                            case 'm':
                            case 'M':
                                if (mainModeBoth == true) {
                                    mainModeBoth = false;
                                }
                                else {
                                    mainModeBoth = true;
                            ...
                            case 'p':
                            case 'P':
                                if (mainModFlags[Config.MOD_PIG] == true) {
                                    mainModFlags[Config.MOD_PIG] = false;
                                }
                                else {
                                    mainModFlags[Config.MOD_PIG] = true;
                                    modFileByLine = manipulate(fileByLine, dictList, mainModFlags);
                                }
                                break;
                            case 'o':
                            case 'O':
                                mainFiles[Config.FILE_OUT] = updateFileName(scnr, mainCurrFileName);
                                break;
                            default:
            ...
            //how to handle different IOExceptions
            }
            catch (Exception e) {
                System.out.println(e);
            }
